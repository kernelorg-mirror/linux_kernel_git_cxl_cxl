...
static void set_pcie_cxl(struct pci_dev *dev)
{
	...
	if (dvsec) {
		...
		pci_read_config_word(dev, dvsec + PCI_DVSEC_CXL_FLEXBUS_PORT_STATUS_OFFSET, &cap);

		dev->is_cxl = FIELD_GET(PCI_DVSEC_CXL_FLEXBUS_PORT_STATUS_CACHE_MASK, cap) ||
			FIELD_GET(PCI_DVSEC_CXL_FLEXBUS_PORT_STATUS_MEM_MASK, cap);
	}
