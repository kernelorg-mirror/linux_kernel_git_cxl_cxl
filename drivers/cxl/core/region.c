 ...
 * available' is a point in time snapshot. If by the time the caller
 * goes to use this root decoder's capacity the capacity is reduced then
 * caller needs to loop and retry.
 *
 * Upon return the root decoder has an elevated reference count that
 * needs to be put with put_device(cxlrd_dev(cxlrd)).
 * cxl_acquire_endpoint() ensures that removal of the root decoder does
 * not proceed until cxl_release_endpoint() at the earliest.
 */
				...
				struct cxl_region_ref *cxl_rr)
{
	 ...
	 * assumption with an assertion. Switch decoders change type based on
	 * what is mapped when they are assigned to a region.
	 */
